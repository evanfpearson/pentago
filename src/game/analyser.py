...
class BoardAnalyser:
    ...
    def __get_notch_row(self, row_num) -> List[Notch]:
    ...
    def __check_vertical_win(self, colour: int, win_length: int):
        for col in range(self.__get_full_size()):
            for i in range(self.__get_full_size() - win_length + 1):
                if all([self.__get_notch_array()[row + i][col].get_colour() == colour for row in range(win_length)]):
                    return True
        return False

    # Check win helpers
    def __check_horizontal_win(self, colour, win_length: int):
        for row in range(self.__get_full_size()):
            for i in range(self.__get_full_size() - win_length + 1):
                if all([self.__get_notch_array()[row][col + i].get_colour() == colour for col in range(win_length)]):
                    return True
        return False

    def __check_diagonal_win(self, colour, win_length: int):
        n = self.__get_full_size()
        w = win_length
        for r_shift in range(n - w + 1):
            for c_shift in range(n - w + 1):
                if any([
                    all([self.__get_notch_array()[r_shift + i][c_shift + i].get_colour() == colour for i in range(w)]),
                    all([self.__get_notch_array()[r_shift + i][n - c_shift - i - 1].get_colour() == colour for i in range(w)])
                ]):
                    return True
        return False

    def __get_full_size(self):
